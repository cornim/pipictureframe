...
def pic_nums_to_display(picture_list, filters):
    for i in range(len(picture_list)):
        ...
        try:
            ...
            log.error(f'Error in file {pic.path}.', exc_info=e)
            continue
        yield i
...
def main():
    ...
    if config.debug_log_file:
        try:
            dfh = RotatingFileHandler(config.debug_log_file, maxBytes=10*1024*1024, backupCount=5, encoding='utf-8')
            dfh.setLevel(logging.DEBUG)
            dfh.setFormatter(formatter)
            log.addHandler(dfh)
        except Exception as e:
            log.error(f'Could not open debug log file {config.debug_log_file}.', exc_info=e)
